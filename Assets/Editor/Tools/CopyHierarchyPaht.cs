...
public class CopyHierarchyPaht : Editor
{
    ...
    [MenuItem("GameObject/CopyHiPathUI",priority =4)]
    public static void CopyHiPathUI()
    {
        targetObj = Selection.activeGameObject;
        UnityEngine.Object obj = Selection.activeObject;
        if (obj == null)
        ...
        if (result != null)
        {
            if (result.Contains("UIMask"))
            {
                var List = result.Split("UIMask");
                var insert = List[1].Insert(0,"UIMask");
                ClipBoard.Copy(insert);
            }
            else
            {
                ClipBoard.Copy(result);
